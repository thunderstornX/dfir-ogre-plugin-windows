...
class RegShellBag(OgrePlugin):
    def description(self) -> PluginDescription:
        return PluginDescription(
            "RegShellBag",
            "Get shellbags (browsed directories) from the UsrClass hive",
        )
    ...
    def build_shell_tree(
        ...
        for bag in bag_key.values():
            ...
            try:
                # take values that have an integer name
                int(name)
            except ValueError:
                continue

            shell_item_list = pyfwsi.item_list()
            shell_item_list.copy_from_byte_stream(bag.data(), ascii_codepage="cp1252")
            items = []
            ...
            try:
                ...
                message = f"while parsing: '{bag_key.name}', error:'{e}'"
...
class ItemChild:
    ...
    def segment(self) -> str:
        ...
        for item in self.items:
            segment += "\\" + self.parse_path_segment(item)
        return segment
    ...
    def parse_path_segment(self, shell_item):
        ...
        if isinstance(shell_item, pyfwsi.control_panel_category):
            ...
            identifier = f"{shell_item.identifier:s}"
            description = WindowsShellFoldersHelper.GetDescription(identifier)
            if description:
                identifier = description
            path_segment = f"<Control panel item: {identifier}>"
            ...
            description = WindowsShellFoldersHelper.GetDescription(
...
class ShellItem:
    ...
    def default_tuple(self, path: str) -> Record:
        ...
        record.add("key_path", value(self.key.path))

        record.add("key_modif_time", value(self.key.mtime))
        record.add(
            "key_security",
            Value.Object(self.key.security_descriptor.to_record()),
        )
        return record

    def write_shell_item(self, shell_item, path: str, output: Output):
        tuple = self.default_tuple(path)

        if isinstance(shell_item, pyfwsi.control_panel_category):
            tuple.add("type", value("control_panel_category"))
            tuple.add("name", value(f"{shell_item.identifier:d}"))
            ...
            if description:
                identifier = description
            tuple.add("type", value("control_panel_item"))
            tuple.add("name", value(f"{identifier}"))

        elif isinstance(shell_item, pyfwsi.network_location):
            tuple.add("type", value("network_location"))
            if shell_item.location:
                tuple.add("name", value(shell_item.location))
            if shell_item.description:
                tuple.add("description", value(shell_item.description))
            if shell_item.comments:
                tuple.add("comments", value(shell_item.comments))

        elif isinstance(shell_item, pyfwsi.root_folder):
            tuple.add("type", value("root_folder"))
            description = WindowsShellFoldersHelper.GetDescription(
                shell_item.shell_folder_identifier
            )
            if description:
            ...
            name = "{0:s}".format(path_segment)
            tuple.add("name", value(name))

        elif isinstance(shell_item, pyfwsi.users_property_view):
            tuple.add("type", value("users_property_view"))
            tuple.add("name", value("<Users property view>"))

        elif isinstance(shell_item, pyfwsi.volume):
            tuple.add("type", value("volume"))
            name = None
            if shell_item.name:
                name = shell_item.name
            elif shell_item.identifier:
                name = "{{{0:s}}}".format(shell_item.identifier)
            tuple.add("name", value(name))
            ...
            for extension_block in shell_item.extension_blocks:
                if isinstance(extension_block, pyfwsi.file_entry_extension):
                    ...
                    if extension_block.localized_name:
                        ...
                        extention_tuple.add("localized_name", value(localized_name))

                    name = extension_block.long_name
                    if not name:
                        name = shell_item.name
                    extention_tuple.add("name", value(name))
                    ...
                    extention_tuple.add("file_reference", value(file_reference))
                    output.write(extention_tuple)

            # no extension found, write default tuple
            if num_extension == 0:
                tuple = self.default_tuple(path)
            ...
            name = "<UNKNOWN: 0x{0:02x}>".format(shell_item.class_type)
            tuple.add("name", value(name))
